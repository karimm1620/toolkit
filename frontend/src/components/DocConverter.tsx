...
export default function DocConverter() {
  ...
  const [type, setType] = useState<'word-to-pdf' | 'pdf-to-word'>('word-to-pdf');
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Terjadi kesalahan saat konversi');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white rounded-xl shadow-sm border border-gray-100">
      <h2 className="text-xl font-semibold mb-4 text-gray-800">Konversi Dokumen</h2>
      
      <div className="flex gap-4 mb-6">
        <button 
          onClick={() => { setType('word-to-pdf'); setFile(null); }}
        ...
        <button 
          onClick={() => { setType('pdf-to-word'); setFile(null); }}
