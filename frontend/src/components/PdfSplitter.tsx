...
export default function PdfSplitter() {
  ...
  const handleSplit = async () => {
    ...
    setLoading(true);
    try {
      // Memanggil fungsi dari api.ts
      const blob = await splitPdf(file);
      
      // Memicu proses unduh file ZIP di browser
      ...
      alert(error instanceof Error ? error.message : 'Terjadi kesalahan saat memisahkan PDF');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white rounded-xl shadow-sm border border-gray-100">
      <h2 className="text-xl font-semibold mb-4 text-gray-800">Pisahkan PDF (Split)</h2>
      <p className="text-gray-500 mb-6 text-sm">
        Pilih satu file PDF. Sistem akan memisahkan setiap halamannya dan mengunduhnya untuk Anda dalam format <strong>.zip</strong>.
      </p>
