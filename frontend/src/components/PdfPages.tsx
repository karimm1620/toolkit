...
export default function PdfPages() {
  ...
  const [action, setAction] = useState<'extract' | 'remove' | 'rotate'>('extract');
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Terjadi kesalahan saat memproses halaman');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white rounded-xl shadow-sm border border-gray-100">
      <h2 className="text-xl font-semibold mb-4 text-gray-800">Manipulasi Halaman PDF</h2>
      ...
      <div className="flex gap-2 mb-6 text-sm">
        {(['extract', 'remove', 'rotate'] as const).map((act) => (
          ...
          >
            {act === 'extract' ? 'Ekstrak' : act === 'remove' ? 'Hapus' : 'Putar'}
          </button>
        ))}
      </div>
      ...
      <div className="mb-4">
        ...
        <input 
          ...
          placeholder={action === 'rotate' ? "Kosongkan untuk memutar semua halaman" : "1, 3, 5-10"}
        ...
        <p className="mt-1 text-xs text-gray-500">
          {action === 'extract' && 'Hanya halaman ini yang akan disimpan dalam PDF baru.'}
          {action === 'remove' && 'Halaman ini akan dihapus dari PDF.'}
          {action === 'rotate' && 'Halaman ini akan diputar sesuai derajat di bawah.'}
        </p>
      </div>

      {action === 'rotate' && (
        <div className="mb-6">
          <label className="block text-sm font-medium text-gray-700 mb-1">Derajat Putaran</label>
          <select 
            value={rotation} 
