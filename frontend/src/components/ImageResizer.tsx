...
export default function ImageResizer() {
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white rounded-xl shadow-sm border border-gray-100">
      <h2 className="text-xl font-semibold mb-4 text-gray-800">Pemrosesan Gambar</h2>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className="col-span-2 sm:col-span-1 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <h3 className="text-sm font-semibold mb-3 text-gray-700">Resize (Opsional)</h3>
          <input 
            type="number" 
        ...
        {/* Bagian Crop */}
        <div className="col-span-2 sm:col-span-1 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <h3 className="text-sm font-semibold mb-3 text-gray-700">Crop Tengah (Opsional)</h3>
          <input 
            type="number" 
            placeholder="Lebar Crop (px)" 
            value={opts.cropWidth || ''} 
            onChange={(e) => setOpts({...opts, cropWidth: Number(e.target.value)})} 
            className="w-full mb-3 p-2 text-sm border border-gray-300 rounded focus:border-blue-500 focus:outline-none" 
          />
          <input 
            type="number" 
            ...
            onChange={(e) => setOpts({...opts, cropHeight: Number(e.target.value)})} 
            className="w-full p-2 text-sm border border-gray-300 rounded focus:border-blue-500 focus:outline-none" 
          />
        </div>

        {/* Rotasi & Format */}
        <div className="col-span-2 p-4 bg-gray-50 rounded-lg border border-gray-200 grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium mb-2 text-gray-700">Rotasi</label>
            <select 
              value={opts.rotate} 
              onChange={(e) => setOpts({...opts, rotate: Number(e.target.value)})} 
              className="w-full p-2 border border-gray-300 text-sm rounded focus:border-blue-500 focus:outline-none"
            >
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-gray-700">Format Output</label>
            <select 
              value={opts.format} 
