...
export default function PdfWatermark() {
  ...
  const [action, setAction] = useState<"watermark" | "pagenumbers">(
    "watermark",
  );
  ...
  const handleProcess = async () => {
    ...
    if (action === "watermark" && !text)
      return alert("Teks watermark tidak boleh kosong");
      ...
      {action === "watermark" && (
        <div className="mb-6">
          ...
          <p className="mt-1 text-xs text-gray-500">
            Teks akan dicetak melintang (diagonal) dengan opasitas 50% di
            seluruh halaman.
          </p>
        </div>
      )}
      ...
      {action === "pagenumbers" && (
        <div className="mb-6">
          <p className="text-sm text-gray-600 bg-blue-50 p-3 rounded border border-blue-100">
            Nomor halaman akan ditambahkan secara otomatis di bagian bawah
            tengah (Bottom-Center) pada setiap halaman dengan format{" "}
            <strong>"Page 1 of X"</strong>.
          </p>
        </div>
      )}
